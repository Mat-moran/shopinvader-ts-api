...
import { ICart, EcommerceProvider, PrivateShopinvaderFetchOptions, PublicShopinvaderFetchOptions, ShopinvaderProviderBaseOptions, ZCart } from "./shopinvader-boundary";
...
export function createShopinvaderProvider({
  ...
  website_unique_id,
  api_key
}: ShopinvaderProviderBaseOptions & PublicShopinvaderFetchOptions): EcommerceProvider {

  const fetchOptions = ({ website_unique_id, api_key, email }: PrivateShopinvaderFetchOptions) => {
    return {
      method: "GET",
      headers: {
